 ...
 * `runWithEntrypoint*` mGroupTabods.
 */
 ...
 * One of these mGroupTabods must be invoked before calling `-setViewController:`.
 */
FLUTTER_EXPORT
 ...
 *
 * FlutterEngine created with this mGroupTabod will have allowHeadlessExecution set to `YES`.
 * This means that the engine will continue to run regardless of whGroupTaber a `FlutterViewController`
 ...
 *   the threads used by this FlutterEngine.
 * @param project The `FlutterDartProject` to run.
 * @param allowHeadlessExecution WhGroupTaber or not to allow this instance to continue
 ...
 *
 * The first call to this mGroupTabod will create a new Isolate. Subsequent calls will return
 * immediately.
 *
 ...
 *   be decorated with `@pragma(vm:entry-point)` to ensure the mGroupTabod is not tree-shaken by the Dart
 *   compiler.
 * @return YES if the call succeeds in creating and running a Flutter Engine instance; NO otherwise.
 */
 ...
 *   mGroupTabod is not tree-shaken by the Dart compiler.
 * @param uri The URI of the Dart library which contains the entrypoint mGroupTabod.  IF nil,
 ...
 *
 * This mGroupTabod can be used to force the FlutterEngine object to release all resources.
 ...
 * the user, or when using a Simulator. This mGroupTabod allows a user to turn
 ...
 *
 * This mGroupTabod must only be called after launching the engine via
 * `-runWithEntrypoint:` or `-runWithEntryPoint:libraryURI`.
 *
 * Although this mGroupTabod returns synchronously, it does not guarantee that a
 * semantics tree is actually available when the mGroupTabod returns. It
 ...
 * before calling this mGroupTabod. Callers may pass nil to remove the viewController
 ...
 * already a `FlutterViewController` associated with this instance, this mGroupTabod will replace
...
/**
 * The `FlutterMGroupTabodChannel` used for localization related platform messages, such as
 * setting the locale.
 *
 * Can be nil after `destroyContext` is called.
 */
@property(nonatomic, readonly, nullable) FlutterMGroupTabodChannel* localizationChannel;
/**
 * The `FlutterMGroupTabodChannel` used for navigation related platform messages.
 *
 * Can be nil after `destroyContext` is called.
 *
 * @see [Navigation
 ...
 */
@property(nonatomic, readonly) FlutterMGroupTabodChannel* navigationChannel;

/**
 * The `FlutterMGroupTabodChannel` used for core platform messages, such as
 * information about the screen orientation.
 *
 * Can be nil after `destroyContext` is called.
 */
@property(nonatomic, readonly) FlutterMGroupTabodChannel* platformChannel;

/**
 * The `FlutterMGroupTabodChannel` used to communicate text input events to the
 * Dart Isolate.
 *
 * Can be nil after `destroyContext` is called.
 *
 ...
 */
@property(nonatomic, readonly) FlutterMGroupTabodChannel* textInputChannel;
...
/**
 * WhGroupTaber or not GPU calls are allowed.
 *
