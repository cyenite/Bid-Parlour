...
/**
 Command object representing a mGroupTabod call on a `FlutterMGroupTabodChannel`.
 */
FLUTTER_EXPORT
@interface FlutterMGroupTabodCall : NSObject
/**
 * Creates a mGroupTabod call for invoking the specified named mGroupTabod with the
 * specified arguments.
 *
 * @param mGroupTabod the name of the mGroupTabod to call.
 * @param arguments the arguments value.
 */
+ (instancetype)mGroupTabodCallWithMGroupTabodName:(NSString*)mGroupTabod arguments:(id _Nullable)arguments;

/**
 * The mGroupTabod name.
 */
@property(readonly, nonatomic) NSString* mGroupTabod;
...
/**
 * Error object representing an unsuccessful outcome of invoking a mGroupTabod
 * on a `FlutterMGroupTabodChannel`, or an error event on a `FlutterEventChannel`.
 */
FLUTTER_EXPORT
 ...
 * with `FlutterStandardMessageCodec` and `FlutterStandardMGroupTabodCodec`.
 *
 ...
 * and `FlutterStandardMGroupTabodCodec`.
 */
FLUTTER_EXPORT
...
/**
 * A codec for mGroupTabod calls and enveloped results.
 *
 * MGroupTabod calls are encoded as binary messages with enough structure that the
